...
struct texture *texture_create(enum texture_kind kind, size_t width, size_t height, bool mipmapping, enum texture_type type, enum texture_format format, enum texture_format_internal format_internal) {
	...
	switch (type) {
	    case TEXTURE_TYPE_FLOAT: {texture->gl_type = GL_FLOAT; }
		                                                   break;
	    case TEXTURE_TYPE_UNSIGNED_BYTE: {texture->gl_type = GL_UNSIGNED_BYTE; }
		                                                                   break;
	}

	// Translate our data formats to OpenGL data formats.
	switch (format) {
	    case TEXTURE_FORMAT_RGB: {texture->gl_format = GL_RGB; }
		                                                   break;
	    case TEXTURE_FORMAT_RGBA: {texture->gl_format = GL_RGBA; }
		                                                     break;
	}

	// Translate our internal formats to OpenGL internal formats.
	switch (format_internal) {
	    case TEXTURE_FORMAT_INTERNAL_RGB: {texture->gl_internal_format = GL_RGB; }
		                                                                     break;
	    case TEXTURE_FORMAT_INTERNAL_RGBA: {texture->gl_internal_format = GL_RGBA; }
		                                                                       break;
	    case TEXTURE_FORMAT_INTERNAL_RGB8: {texture->gl_internal_format = GL_RGB8; }
		                                                                       break;
	    case TEXTURE_FORMAT_INTERNAL_RGBA8: {texture->gl_internal_format = GL_RGBA8; }
		                                                                         break;
	    case TEXTURE_FORMAT_INTERNAL_RGB16F: {texture->gl_internal_format = GL_RGB16F; }
		                                                                           break;
	    case TEXTURE_FORMAT_INTERNAL_RGBA16F: {texture->gl_internal_format = GL_RGBA16F; }
		                                                                             break;
	    case TEXTURE_FORMAT_INTERNAL_RGB32F: {texture->gl_internal_format = GL_RGB32F; }
		                                                                           break;
	    case TEXTURE_FORMAT_INTERNAL_RGBA32F: {texture->gl_internal_format = GL_RGBA32F; }
		                                                                             break;
	}
...
struct texture *texture_create_from_memory(enum texture_kind kind, const unsigned char *data, size_t size) {
	...
	switch (components) {
	    case 3: {format = TEXTURE_FORMAT_RGB; }
		                                  break;
	    case 4: {format = TEXTURE_FORMAT_RGBA; }
		                                   break;
	    default:
